...
`source` says where the token came from: `argv`, `argv_write`, `redirect_read`, `redirect_write` or `env_dump`. `argv` is vague on purpose. `cat X` reads and `tee X` writes, and telling those apart needs per-command knowledge the database does not carry for a positional. Two sources do know the direction. A redirect knows it because the operator says so, and `argv_write` is the value of an option the tool documents as naming a file it writes, such as `curl -o` — so `curl -o ~/.ssh/authorized_keys https://x` reports a write while `cat ~/.ssh/authorized_keys` reports an argument of unknown direction.

A marked option's value is reported *only* as `argv_write`, so selecting `source == "argv"` no longer sees it — `curl -o ~/.ssh/authorized_keys` has one hit and it is not an `argv` one. And `argv_write` never covers a destination written as a plain positional (`cp a b`, `tee out.txt`), an operand that is not an option (`dd of=X`), or an option whose direction depends on another option (`tar -f`, `tar -C`). Read it as an extra signal, never as the definition of "this writes" — `source != "argv_write"` does not mean nothing is written. [SPEC.md](SPEC.md) has the full list.
...
- **File path detection** -- `write_paths` holds redirect targets and the values of options the database marks as naming an output (`curl -o`, `wget -O`, `cp -t`, `sort -o`), `read_paths` holds input redirect targets; a write is `>` or `>>` with the optional file descriptor and `|` no-clobber override bash allows (`1>`, `2>>`, `>|`), plus `&>`, `&>>` and `>&`; writes to `/tmp` and `/var/tmp` stay at LOW risk. A destination named by a plain positional (`cp a b`, `tee out.txt`) is in neither field
